...
export default function CategoryDetail() {
  ...
  const handleDeleteItem = async (item: any) => {
    Alert.alert('移除', `确定要把 "${item.name}" 移除吗？`, [
      { text: '取消', style: 'cancel' },
      { text: '甩', style: 'destructive', onPress: async () => {
  ...
  const saveEdit = async () => {
    ...
    const { error } = await supabase.from('items').update({ 
      name: editName, quantity: parseInt(editQuantity) || 0, price: editPrice, description: editDescription            
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={styles.container}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.inputSection}>
          <Text style={styles.inputLabel}>✨ 登记新物品 (长按可编辑，左滑更多选项)</Text>
        ...
        <Modal visible={isModalVisible} transparent={true} animationType="fade">
          <View style={styles.modalOverlay}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContent}>
              ...
              <TextInput style={[styles.modalInput, styles.textArea]} value={editDescription} onChangeText={setEditDescription} placeholderTextColor="#D7CCC8" multiline={true} numberOfLines={3} />
